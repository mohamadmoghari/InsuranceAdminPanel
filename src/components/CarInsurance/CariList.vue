    ...
    <section class="table-responsive">
      <table class="table table-stripped mb-0">
        ...
        <tr>
          ...
          <th width="150">بیمه ثالث</th>
          <th width="150">تعداد بیمه ثالث</th>
          <th width="150">بیمه بدنه</th>
          <th width="150">تعداد بیمه بدینه</th>
          <th width="150">تشخیص</th>
          <th width="150">خسارت</th>
        ...
        <tr v-for="singleInsurance in insuranceLists" :key="singleInsurance.id">
          ...
          <td>{{ singleInsurance.sDate }}</td>
          <td>{{ singleInsurance.eDate }}</td>
          <td>{{ singleInsurance.hasSalesBime }}</td>
          <td>{{ singleInsurance.salesBimeCount }}</td>
          <td>{{ singleInsurance.hasBadaneBime }}</td>
          <td>{{ singleInsurance.badaneBimeCount }}</td>
          <td>{{ singleInsurance.hasTaghsit }}</td>
          <td>{{ singleInsurance.hasKhesarat }}</td>
...
export default {
  ...
  methods: {
    getAllInsuranceList() {
      this.loading = true
      this.$store.dispatch('CarInsurance/getAllCarInsuranceList', this.filter)
        .then(response => {
    ...
    removeSingleInsurance() {
      ...
      this.$store.dispatch('CarInsurance/deleteSingleCarInsurance', this.removeSingleInsuranceId)
        .then(response => {
