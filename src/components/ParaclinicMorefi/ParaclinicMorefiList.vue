    ...
    <section class="table-responsive">
      <table class="table table-stripped mb-0">
        ...
        <tr v-for="singleParaclinic in paraclinicLists" :key="singleParaclinic.id">
          ...
          <td>{{ singleParaclinic.subject }}</td>
          <td>{{ singleParaclinic.count }}</td>
...
export default {
  ...
  methods: {
    getAllParaclinicList() {
      this.loading = true
      this.$store.dispatch('ParaclinicMorefi/getAllParaclinicMorefiList', this.filter)
        .then(response => {
    ...
    removeSingleParaclinic() {
      ...
      this.$store.dispatch('ParaclinicMorefi/deleteSingleParaclinicMorefi', this.removeSingleTravelId)
        .then(response => {
